                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <div>
                                    ...
                                    <select value={source.treatmentMethod} onChange={e => onUpdate({ treatmentMethod: e.target.value as TreatmentMethod })} className={commonSelectClass}>
                                        {source.wasteType && fuels.activity[source.wasteType] && Object.keys(fuels.activity[source.wasteType]).map(method => <option key={method} value={method}>{t(fuels.activity[source.wasteType][method].translationKey)}</option>)}
                                    </select>
                                </div>
                            </div>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                            ...
                                            <div className="sm:col-span-2">
                                                ...
                                                <select value={source.vehicleType} onChange={e => onUpdate({ vehicleType: e.target.value })} className={commonSelectClass}>
                                                    {source.transportMode && transportFuels[source.transportMode] && Object.keys(transportFuels[source.transportMode]).map(vehicle => <option key={vehicle} value={vehicle}>{t(transportFuels[source.transportMode][vehicle].translationKey)}</option>)}
                                                </select>
                                            </div>
